...
async fn test_new_step_factor() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for event_id in event_ids {
        ...
        let detailed_scores_vec: Vec<DetailedScore> = serde_json::from_str(
            match event_id {
        ...
        // Load summary scores
        let summary_scores_obj: serde_json::Value = serde_json::from_str(
            match event_id {
        ...
        if event_id == 401580355 {
            ...
            assert_eq!(global_step_factor, 4.5, "Expected global step factor to be 4.5 for event 401580355");
        ...
        // STEP 6: For event 401703504, verify per-player step factors
            ...
            assert!(!result.results.is_empty(), "Could not find Player1 with step factor");
            ...
            assert_eq!(step_factor, 1.0, "Expected Player1 to have step_factor 1.0 in event 401703504");
