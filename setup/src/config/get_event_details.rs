...
pub(crate) fn build_get_event_details_mode(cli: &Cli, file_config: &FileConfig) -> Result<AppMode> {
    ...
    if !one_shot {
        return Err(anyhow!(
            "--one-shot is required for --mode=get_event_details"
        ));
    }
    ...
    let output_json_stdout =
        cli.output_json_stdout || file_config.output_json_stdout.unwrap_or(false);
    ...
    let event_ids = event_id_input.as_deref().map(parse_event_ids).transpose()?;
