...
impl ServerlessStorage {
    ...
    fn kv_test_lock_key(event_id: i32) -> String {
        format!("event:{event_id}:test_lock")
    }

    fn kv_test_lock_prefix() -> &'static str {
        "event:"
    }
    ...
    pub async fn admin_test_lock(
        ...
        match mode {
            TestLockMode::Shared => {
                ...
                doc.shared_holders
                    .insert(token.to_string(), expires_at_str);
            ...
            TestLockMode::Exclusive => {
                ...
                doc.exclusive_holder = Some((token.to_string(), expires_at_str));
                self.kv_put_json(&key, &doc).await?;
                Ok((true, true))
            }
        }
...
#[derive(Serialize, Deserialize)]
struct TestLockDoc {
    shared_holders: HashMap<String, String>,
    exclusive_holder: Option<(String, String)>,
}
